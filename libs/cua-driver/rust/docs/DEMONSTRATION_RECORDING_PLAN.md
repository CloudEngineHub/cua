...
This generalizes the `cua skills record` CLI feature (TS/Python, browser/VNC-based) and the Codex
computer-use demonstration-skill format down into cua-driver itself, so it works for every harness
(Claude Code, Codex, OpenClaw, OpenCode, raw MCP).
...
- Bundle `Skills/cua-driver/demonstration-to-skill/SKILL.md`: how to read `TRAJECTORY.md` and author
  a good SKILL.md (Codex-style: frontmatter, Steps with Context/Intent/Expected Result, Agent
